...
try {
    ...
    foreach ($tablesToFix as $table) {
        ...
        if (in_array('update_at', $columnNames)) {
            ...
            if ($updateAtColumn) {
                // 构建新字段的定义
                $newColumnDef = "`updated_at` {$updateAtColumn['Type']} ";
                $newColumnDef .= $updateAtColumn['Null'] === 'NO' ? 'NOT NULL ' : 'NULL ';
                $newColumnDef .= $updateAtColumn['Default'] !== null ? "DEFAULT '{$updateAtColumn['Default']}' " : '';
                $newColumnDef .= $updateAtColumn['Extra'];
                ...
                try {
                    ...
                    if (!in_array('updated_at', $columnNames)) {
                        $pdo->exec("ALTER TABLE $table ADD COLUMN $newColumnDef");
    ...
    try {
        ...
        echo "   - article_read_logs 表 updated_at 查询测试: ✓ 成功\n";
    } catch (Exception $e) {
    ...
    try {
        ...
        echo "   - sys_news_article 表 updated_at 查询测试: ✓ 成功\n";
    } catch (Exception $e) {
    ...
    try {
        ...
        echo "   - article_read_statistics 表 updated_at 查询测试: ✓ 成功\n";
    } catch (Exception $e) {

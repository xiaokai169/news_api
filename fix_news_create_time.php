...
try {
    ...
    if ($hasCreateAt && !$hasCreateTime) {
        echo "方案1: 重命名字段 (推荐)\n";
        echo "  将 create_at 重命名为 create_time\n";
        echo "  将 update_at 重命名为 update_time\n\n";
    } elseif (!$hasCreateAt && $hasCreateTime) {
        echo "方案1: 修改实体映射 (推荐)\n";
        echo "  将实体中的字段映射改为 create_at 和 update_at\n\n";
    } elseif ($hasCreateAt && $hasCreateTime) {
        echo "方案1: 数据同步 (推荐)\n";
        echo "  将 create_at 数据同步到 create_time\n";
        echo "  将 update_at 数据同步到 update_time\n\n";
    }
